...
WHERE length(cleaned_word) > 3
    ...
    AND cleaned_word NOT IN (
        ...
        'work','first','well','even','want','because','these','give','most','were',
        'been','here','many','dont','does','more','less'
    )
GROUP BY cleaned_word
ORDER BY avg_engagement DESC
LIMIT 50;
-- name: GetWordCloudEngagementDataViews :many
WITH post_engagement AS (
    SELECT p.id,
        p.content,
        COALESCE(prh.views, 0) as engagement
    FROM posts p
        JOIN sources s ON p.source_id = s.id
        LEFT JOIN (
            SELECT DISTINCT ON (post_id) post_id,
...
ORDER BY avg_engagement DESC
LIMIT 50;
-- name: GetWordCloudEngagementDataViewsFiltered :many
WITH post_engagement AS (
    SELECT p.id,
        p.content,
        COALESCE(prh.views, 0) as engagement
