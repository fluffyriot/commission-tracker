...
const getWordCloudEngagementData = `-- name: GetWordCloudEngagementData :many
...
type GetWordCloudEngagementDataRow struct {
	Word            string  `json:"word"`
...
func (q *Queries) GetWordCloudEngagementData(ctx context.Context, userID uuid.UUID) ([]GetWordCloudEngagementDataRow, error) {
	rows, err := q.db.QueryContext(ctx, getWordCloudEngagementData, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const getWordCloudEngagementDataViews = `-- name: GetWordCloudEngagementDataViews :many
WITH post_engagement AS (
    SELECT p.id,
        p.content,
...
type GetWordCloudEngagementDataViewsRow struct {
	Word            string  `json:"word"`
	TotalEngagement int64   `json:"total_engagement"`
	AvgEngagement   float64 `json:"avg_engagement"`
	UsageCount      int64   `json:"usage_count"`
...
func (q *Queries) GetWordCloudEngagementDataViews(ctx context.Context, userID uuid.UUID) ([]GetWordCloudEngagementDataViewsRow, error) {
	rows, err := q.db.QueryContext(ctx, getWordCloudEngagementDataViews, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetWordCloudEngagementDataViewsRow
	for rows.Next() {
		var i GetWordCloudEngagementDataViewsRow
		if err := rows.Scan(
			&i.Word,
			&i.TotalEngagement,
			&i.AvgEngagement,
...
const getWordCloudEngagementDataViewsFiltered = `-- name: GetWordCloudEngagementDataViewsFiltered :many
WITH post_engagement AS (
    SELECT p.id,
        p.content,
        COALESCE(prh.views, 0) as engagement
...
type GetWordCloudEngagementDataViewsFilteredParams struct {
	UserID    uuid.UUID    `json:"user_id"`
	StartDate sql.NullTime `json:"start_date"`
	EndDate   sql.NullTime `json:"end_date"`
	PostTypes []string     `json:"post_types"`
...
func (q *Queries) GetWordCloudEngagementDataViewsFiltered(ctx context.Context, arg GetWordCloudEngagementDataViewsFilteredParams) ([]GetWordCloudEngagementDataViewsFilteredRow, error) {
	rows, err := q.db.QueryContext(ctx, getWordCloudEngagementDataViewsFiltered,
		arg.UserID,
		arg.StartDate,
		arg.EndDate,
		pq.Array(arg.PostTypes),
	...
	defer rows.Close()
	var items []GetWordCloudEngagementDataViewsFilteredRow
	for rows.Next() {
		var i GetWordCloudEngagementDataViewsFilteredRow
		if err := rows.Scan(
			&i.Word,
			&i.TotalEngagement,
			&i.AvgEngagement,
